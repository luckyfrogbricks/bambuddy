...
describe('SpoolBuddyDashboard', () => {
  ...
  describe('auto-close countdown after a barcode add', () => {
    it('starts a cancellable Close countdown when the scale empties right after the add', async () => {
      const { api, spoolbuddyApi } = await import('../../api/client');
      (spoolbuddyApi.getDevices as ReturnType<typeof vi.fn>).mockResolvedValue([
        { device_id: 'dev-1', has_barcode: true, barcode_enabled: true },
      ]);
      ...
      const newSpool = {
        id: 4, material: 'PLA', brand: 'Polymaker', tag_uid: 'AABB99', tray_uuid: null,
        archived_at: null, color_name: 'Charcoal', rgba: '3B3B3FFF', subtype: null,
        label_weight: 1000, core_weight: 250, weight_used: 0,
      };
      const baseSpools = await (api.getSpools as ReturnType<typeof vi.fn>)();
      // First load: the roll is unknown; after the add the refetch returns it.
      (api.getSpools as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce(baseSpools)
        .mockResolvedValue([...baseSpools, newSpool]);
      ...
      function StatefulWrapper() {
        const [ctx, set] = React.useState<unknown>({
          ...mockOutletContext,
          sbState: { ...baseSbState, unknownTagUid: 'AABB99', weight: 1247, weightStable: true, lastScan: scan },
        });
        setCtx = set;
        return <Outlet context={ctx} />;
      }
      ...
      // The modal auto-opened on the scan; add the spool (the UnknownTagCard
      // behind the modal has an identically-named button, so scope the click
      // to the modal overlay).
      const confirmTitle = await screen.findByText('Confirm New Spool');
      const modalRoot = confirmTitle.closest('.fixed') as HTMLElement;
      fireEvent.click(within(modalRoot).getByRole('button', { name: /^Add to Inventory$/i }));
      await waitFor(() => expect(api.createSpool).toHaveBeenCalledTimes(1));

      // Roll lifted off the scale within the arm window → countdown starts on
      // the Current Spool card's Close button.
      act(() => {
        setCtx({
          ...mockOutletContext,
          sbState: { ...baseSbState, unknownTagUid: null, weight: 5, weightStable: true, lastScan: null },
        });
      });
      expect(await screen.findByText('Closing in 3…')).toBeTruthy();

      // The X cancels the countdown and leaves a plain Close button.
