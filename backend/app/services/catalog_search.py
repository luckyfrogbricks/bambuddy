...
from backend.app.services.barcode_resolver import BARCODE_FIELD_KEYS, barcode_lookup_enabled
...
async def search_catalog(
    ...
    if spoolman_client is not None:
        ...
        for spool in result.scalars().all():
            if catalog_match(tokens, spool.brand, spool.material, spool.subtype, spool.color_name):
                rows.append(
                    CatalogSearchRow(
                        source="inventory",
                        spool_id=spool.id,
